...
**Branch override**:
The post-pick prompt that fires when the user's current branch (the one they ran `tide run` from) differs from the picked **PRD**'s or **Standalone Issue**'s Linear-auto-generated `branchName`. Rendered as a clack `select` between two options — Linear's `branchName` (default) and the user's current branch — and chooses which branch the **Feature worktree** is created on. No prompt fires when current branch == Linear's `branchName` (silent reuse). No `tide.config.ts` notion of "base branches"; the prompt is the user's only escape hatch for which branch the agent commits to. Replaces today's hard pre-flight gate that errored out when current branch == Linear `branchName` (`src/cli/run.ts`).

The override decision is independent of the **PR target branch**: regardless of which option the user picks, the eventual PR merges into the **PR target branch** resolved separately (default `origin/HEAD`, user-confirmed when current branch differs). Picking "current" for a hand-named branch and accepting the default `master` for the **PR target branch** is the typical override path.
...
**PR target branch**:
The branch the eventual PR merges into, surfaced to the **agent** as `{{BASE_BRANCH}}` in tide-owned prompts and passed to `gh pr create --base`. Resolved at `tide run` startup from `origin/HEAD` (the remote's default branch, set by `git clone` and queryable via `git rev-parse --abbrev-ref origin/HEAD`). When the user's current branch == `origin/HEAD` (typical: invoked from trunk), tide proceeds silently. When they differ (typical: user invoked from a hand-named feature branch on the **Branch override** path), tide prompts the user to confirm the auto-detected target or supply their own. A user-supplied override must resolve to a local ref (`git rev-parse --verify`) — tide never auto-fetches on the user's behalf; missing local refs error and re-prompt with the `git fetch` hint. When `origin/HEAD` is unset, the prompt fires with no default.

The **PR target branch** is distinct from the user's current branch. The two were conflated in the initial **Branch override** implementation (a single `resolveBaseBranch` capture filled both roles), which silently broke the override path: a user picking their hand-named branch as the feature got `featureBranch === baseBranch`, the rev-list gate skipped PR creation, and they never saw the override-active warning. The split was introduced after PR #42 to make the override path actually produce a PR.
_Avoid_: "base branch" without qualification (collides with sandcastle's `baseBranch` strategy-input field), "PR base" (informal — "PR target branch" is tide's domain term).
...
**Repo prefix**:
The `[<repo>] ` token (open-bracket, repo name, close-bracket, single space) at the start of every Linear issue title that tide will see. `<repo>` is the GitHub repo name returned by `gh repo view --json name` for the working tree (same source as the rest of tide's repo identity per `src/gh-identity/index.ts`). Lets one Linear team back multiple repos: tide filters every Linear list query by `title: { startsWith: "[<repo>] " }`, so wrong-repo and unprefixed issues are invisible to the picker, the queue build, and the **queue rebuild**. The **Bundled skills** `tide-triage`, `tide-to-prd`, and `tide-to-issues` are responsible for writing the prefix at issue-creation time; tide is responsible for filtering on it at every fetch.
_Avoid_: "repo tag" (collides with git tags), "repo bracket" (form, not function).

**Bundled skill**:
A `.claude/skills/tide-*/` directory tide writes into the consuming repo from a template embedded in the binary at `src/cli/setup-templates/claude-skills/`. Tide owns these files end-to-end: `tide setup` overwrites them every run (silent if bytes match, logged path on change), they are committed to the consuming repo (not gitignored), and they update by re-running `tide setup` against a newer tide version. The three current bundled skills are `tide-to-prd`, `tide-to-issues`, and `tide-triage` — invoked as `/tide-to-prd` etc. The hyphen prefix is naming convention, not a Claude Code plugin namespace (see ADR-0015 for why a real `tide:` namespace was rejected). User-authored skills under `.claude/skills/` outside the `tide-*/` namespace are never touched by `tide setup`.
_Avoid_: "tide skill" (collides with skills the user might author specifically for tide), "shipped skill" (too generic), "embedded skill" (collides with the binary-embedding tech term).
